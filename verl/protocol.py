...
class DataProto:
    ...
    def split_by_ratio(self, ratio: float) -> List["DataProto"]:
        """
        Split the DataProto into two parts according to the given ratio.

        Args:
            ratio (float): the ratio for the first split (e.g., 0.6 means 60% and 40%)

        Returns:
            List[DataProto]: [first_chunk, second_chunk]
        """
        total_size = len(self)
        first_size = int(total_size * ratio)
        
        # slice first and second parts
        if self.batch is not None:
            first_batch = self.batch[:first_size]
            second_batch = self.batch[first_size:]
        else:
            first_batch = None
            second_batch = None

        first_non_tensor_batch = {}
        second_non_tensor_batch = {}
        for key, value in self.non_tensor_batch.items():
            assert isinstance(value, np.ndarray)
            first_non_tensor_batch[key] = value[:first_size]
            second_non_tensor_batch[key] = value[first_size:]

        first_proto = DataProto(batch=first_batch, non_tensor_batch=first_non_tensor_batch, meta_info=self.meta_info)
        second_proto = DataProto(batch=second_batch, non_tensor_batch=second_non_tensor_batch, meta_info=self.meta_info)
