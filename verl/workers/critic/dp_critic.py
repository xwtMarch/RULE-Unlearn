...
class DataParallelPPOCritic(BasePPOCritic):
    ...
    def _forward_micro_batch(self, micro_batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if self.config.padding_free:
            ...
            # after pad_input: values: (bsz, seqlen, 2)
            values = values[:, -response_length - 1 : -1]  # (bsz, response_length, 2)
        else:
            output = self.critic_module(
                input_ids=input_ids,
            ...
            values = values[:, -response_length - 1 : -1].squeeze(-1)  # (bsz, response_length, 2)
    ...
    def compute_values(self, data: DataProto) -> torch.Tensor:
        ...
        for micro_batch in micro_batches:
            ...
            values = self._forward_micro_batch(model_inputs) # values: (bsz, response_length, 2)
            values_lst.append(values)
        
        values = torch.concat(values_lst, dim=0) 
        responses = data.batch["responses"]
        attention_mask = data.batch["attention_mask"] # attention_mask: (bsz, seqlen)
        response_length = responses.size(1)
