...
from verl.eval.eval_rwku import eval_forget, eval_neighbor
...
from ..utils.dataset import RLHFDataset, collate_fn, Target2Name
...
class RayPPOTrainer:
    ...
    def _create_dataloader(self):
        ...
        if "eval_data" in self.config.trainer.experiment_name:
            ...
            val_dataset = val_dataset.map(
                lambda x: {**x, self.config.data.prompt_key: x[self.config.data.prompt_key] + " Answer:"}
            )
    ...
    def _validate(self):
        ...
        sample_inputs, sample_outputs, sample_scores, sample_labels = [], [], [], []
        ...
        forget_val_dir = "/".join(self.config.data.val_files.split("/")[:-1])
        ...
        save_dir = os.path.join(
            ...
            *self.config.data.val_files.split("/")[1:-1],
            ...
            'results': [
                {
                    ...
                    "label": l,
                    "reward_score": s
                } for q, a, s, l in zip(sample_inputs, sample_outputs, sample_scores, sample_labels)
            ]
    ...
    def init_workers(self) -> None:
        ...
        if self.use_rollout:
            ...
            actor_rollout_ref_cls = RayClassWithInitArgs(
                cls=self.role_worker_mapping[Role.ActorRolloutRef], config=self.config.worker, role="rollout"
            )
            self.resource_pool_to_cls[resource_pool]["rollout"] = actor_rollout_ref_cls
    ...
    def fit(self):
        ...
        for _ in tqdm(range(episodes), desc="Episode", position=0):
